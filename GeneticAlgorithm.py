...
class GeneticAlgorithm:
    show_crossover_details = False
    show_fitness_details = False
    show_crossover_details = False
    show_mutation_details = False
    show_if_duplicates = False
    silent = True
    mean_time = None
    mean_fitness = None
    fitness_values = None

    def __init__(self, test_case_matrix, chromosome_size, population_size, number_of_generations, crossover_rate, mutation_rate, tournament_size_percent, strongest_winner_probability):
        self.test_case_matrix = test_case_matrix
    ...
    def generate_population(self, population_size, chromosome_size):
        ...
        for i in range(0, population_size):
            ...
            if self.show_if_duplicates: print()
            population.append(chromosome)
        return population

    def populate(self, j, chromosome):
        random_index = random.randint(0, len(self.test_case_matrix) - 1)
        chromosome.append(self.test_case_matrix[random_index])
        if self.show_if_duplicates: print("Chromosome:", [i[0] for i in chromosome])
        if j > 0:
            if self.check_for_duplicate_genes(chromosome):
                if self.show_if_duplicates: print("Duplicate found, popping it and picking again")
                chromosome.pop()
                self.populate(j, chromosome)

    def check_for_duplicate_genes(self, chromosome):
        duplicate_checker = []
    ...
    def tournament_selection(self, population):
        winners = []
        for round_ in range(0, self.tournament_rounds()):
            participants = []
    ...
    def fitness(self, chromosome, chromosome_size):
        ...
        if self.show_fitness_details: print("Calculating fitness (APFD) of Chromosome:", [i[0] for i in chromosome])
    ...
    def crossover(self, first_parent, second_parent):
        if self.show_crossover_details:
    ...
    def mutate(self, generation):
        ...
        for chromosome in generation:
            if self.show_mutation_details: print("\nChromosome being worked on:  ", [i[0] for i in chromosome], "\n")
            for test_case in chromosome:
                if self.decision(self.mutation_rate):
                    if self.show_mutation_details: print("Chromosome pre-mutation:   ", [i[0] for i in chromosome])
    ...
    def run(self, number_of_runs):
        ...
        for i in range(0, number_of_runs):
            ...
            if self.show_crossover_details: print("       Fitness (APFD)                          Chromosome        "
                                                "             Generation\n")
